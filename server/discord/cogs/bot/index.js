...
module.exports.info = {
	...
	description: 'Get information on a bot or a user from bots.discord.pw or discordbots.org.',
	...
	use: [
		{
			name: '<user>',
			value: 'Fetch all bots this user have submitted to the bot websites'
		},
		{
			name: '<bot>',
			value: 'Get information about the bot on the two bot websites'
		}
	]
};
...
module.exports.command = (message) => {
	...
	if (!id || !id[1]) {
		...
		if (!user) {
			message.channel.createMessage(`The user is not in a guild with ${config.get('name')}, or is offline in a really large guild.`);
		} else if (user.bot) {
			const bdpw = {
				method: 'GET',
				json: true,
