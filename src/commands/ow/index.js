...
module.exports = function manpages(message) {
	...
	request(owUrl, function (error, response, body) {
		if (response.statusCode === 404) {
			return message.reply("No player tag was found.");
		} else if (response.statusCode === 200){
			var owData = JSON.parse(body);
			return message.reply(owData["username"] + " Ranked #" + owData["competitive"]["rank"] + " W/L " + owData["games"]["competitive"]["wins"] + "/" + owData["games"]["competitive"]["lost"]);
		}
	});
}
